 ...
 * @returns ratio
 */
export function weightToSwlRatio(swl: number, weight: number): number {
 ...
 */
export function breakingStrength(swl: number, safetyFactor: number): number {
 ...
 */
export function areaRectangular(height: number, width: number): number {
  const params = z
    .object({
      height: z.number(),
      width: z.number(),
 ...
 */
export function volumeCylinder(diameter: number, height: number): number {
  const zHeight = z.number().positive().parse(height);
